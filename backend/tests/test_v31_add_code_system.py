...
class TestV31AddCodeSystem:
    ...
    def test_get_my_add_code(self, api_client, base_url):
        ...
        assert len(code) == 11, f"Code should be 11 chars (FUNK-XXXXXX), got: {len(code)}"
        
        code_part = code.split("-")[1]
        assert len(code_part) == 6, f"Code part should be 6 chars, got: {len(code_part)}"
        assert code_part.isalnum(), f"Code part should be alphanumeric, got: {code_part}"
    ...
    def test_reset_add_code(self, api_client, base_url):
        ...
        # Verify format
        assert new_code.startswith("FUNK-"), f"New code should start with 'FUNK-', got: {new_code}"
    ...
    def test_add_by_code_send_request(self, api_client, base_url):
        ...
        response = api_client.post(f"{base_url}/api/contacts/add-by-code", json={
            "code": "FUNK-ADL3R2"
        })
        ...
        if response.status_code == 200:
            ...
            print(f"✓ Expected 400: {data.get('detail', 'Already sent or already contacts')}")
    ...
    def test_get_contact_requests(self, api_client, base_url):
        ...
        if len(incoming) > 0:
            req = incoming[0]
            print(f"  First incoming: {req.get('requester_username')} → {req.get('status')}")
            assert "id" in req
    ...
    def test_accept_contact_request(self, api_client, base_url):
        ...
        # Accept request
        response = api_client.post(f"{base_url}/api/contacts/request/{request_id}/accept")
    ...
    def test_get_contacts_only_confirmed(self, api_client, base_url):
        ...
        for contact in contacts:
            ...
            assert "add_me_code" not in contact, "add_me_code should not be exposed in contacts list"
    ...
    def test_reject_contact_request(self, api_client, base_url):
        ...
        add_res = api_client.post(f"{base_url}/api/contacts/add-by-code", json={
            "code": "FUNK-ADL3R2"
        })
        ...
        # Login as adler-2
        login_res2 = api_client.post(f"{base_url}/api/auth/login", json={
            "username": "adler-2",
            "passkey": "Funk2024!"
        })
        ...
        # Reject request
        response = api_client.post(f"{base_url}/api/contacts/request/{request_id}/reject")
    ...
    def test_full_add_code_flow(self, api_client, base_url):
        ...
        login1 = api_client.post(f"{base_url}/api/auth/login", json={
            "username": "wolf-1",
            "passkey": "Funk2024!"
        })
        ...
        login2 = api_client.post(f"{base_url}/api/auth/login", json={
            "username": "adler-2",
            "passkey": "Funk2024!"
        })
        ...
        if len(incoming) > 0:
            ...
            request_id = incoming[0]["id"]
            accept_res = api_client.post(f"{base_url}/api/contacts/request/{request_id}/accept")
