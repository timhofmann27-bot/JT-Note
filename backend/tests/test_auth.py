...
class TestAuth:
    ...
    def test_login_admin_success(self, api_client, base_url, admin_credentials):
        ...
        assert "email" not in data["user"], "Email field should not exist in anonymous auth"
        print(f"✓ Admin login successful: {data['user']['name']} (@{data['user']['username']})")
    ...
    def test_login_test_user_success(self, api_client, base_url, test_user_credentials):
        ...
        assert "email" not in data["user"], "Email field should not exist in anonymous auth"
        print(f"✓ Test user login successful: {data['user']['name']} (@{data['user']['username']})")
    ...
    def test_register_new_user(self, api_client, base_url):
        ...
        token = data["token"]
        print(f"✓ User registered: @{data['user']['username']} ({data['user']['name']})")
    ...
    def test_get_me_authenticated(self, api_client, base_url, admin_token):
        ...
        print(f"✓ User info retrieved: {data['user']['name']} (@{data['user']['username']})")
    ...
    def test_change_passkey(self, api_client, base_url):
        ...
        # Change passkey
        api_client.headers.update({"Authorization": f"Bearer {token}"})
        change_response = api_client.post(f"{base_url}/api/auth/change-passkey", json={
    ...
    def test_users_endpoint_no_email(self, api_client, base_url, admin_token):
        ...
        for user in data["users"]:
            assert "email" not in user, f"User {user.get('username')} has email field (should not exist)"
